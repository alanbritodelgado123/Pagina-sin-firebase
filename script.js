...
class QuizApp {
  ...
  updateHomeGreeting() {
    ...
    if (greetingEl) greetingEl.textContent = `¡Hola, ${this.currentUsername}!`;
  }
  ...
  showNotification(message, type = 'info', duration = 3000) {
    if (this.notificationActive) return; // <--- Evita mostrar otra si ya hay una activa
    this.notificationActive = true;      // <--- Marca como activa
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        this.notificationActive = false; // <--- Libera la bandera al cerrar
      }, 300);
    }, duration);
  }
  ...
  initializeCanvas() {
    ...
    const animateParticles = () => {
      ...
      particles.forEach(p => {
        ...
        ctx.fillStyle = document.body.classList.contains('light-mode') ? '#5dade2' : '#6C5CE7';
        ctx.beginPath();
...
class AudioSystem {
  constructor() {
    this.enabled = true;
    this.volume = 0.7; // Volumen inicial (0.0 a 1.0)
    this.sounds = {
    ...
    <div class="setting-group stats-card">
      ...
      <div class="stats-grid">
        ...
        <div class="stat-item">
          ...
          <span class="stat-value">${this.gameStats.bestScore}</span>
        </div>
        <div class="stat-item">
          <span class="stat-icon">⏱️</span>
          <span class="stat-label">Tiempo promedio</span>
          <span class="stat-value">${(this.gameStats.averageTime / 1000).toFixed(1)} seg</span>
        </div>
      </div>
    </div>
  `;
